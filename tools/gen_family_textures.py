...
def t_stripped_petrified_tuning_wood_top() -> Canvas:
    """The unstripped top's own end grain, with only the perimeter softened.

    PLAYER: "the top log textures dont change, keep the original log tops, just soften
    the colors on the perimeter pixels a bit to reflect the stripped logs."

    So this is the SAME log_rings call as t_petrified_tuning_wood_top - same seed, same ring tones, same
    heartwood - on the same bark ramp. The only difference is the 1px border, which
    moves off the dark bark tones and up into the wood's own body, because that ring
    IS the bark and a stripped log has none.

    It replaces a bespoke `stripped_log_rings` pattern that redrew the whole face and
    so shared nothing with the log it came from - 244 to 256 of the 256 pixels differed
    from the unstripped top, which is not "stripped", it is a different tree.
    """
    c = Canvas(TUNE_RAMP)
    log_rings(c, 8203, dark=2, bark_dark=3, bark_mid=4, heart=8)
    return c
...
def t_stripped_humming_stem_top() -> Canvas:
    ...
    So this is the SAME log_rings call as t_humming_stem_top - same seed, same ring tones, same
    heartwood - on the same bark ramp. The only difference is the 1px border, which
    moves off the dark bark tones and up into the wood's own body, because that ring
    IS the bark and a stripped log has none.
    ...
    """
    c = Canvas(HUM_STEM_RAMP)
    log_rings(c, 8331, dark=2, bark_dark=3, bark_mid=4, heart=7)
    return c
...
def t_stripped_echo_ash_log_top() -> Canvas:
    ...
    So this is the SAME log_rings call as t_echo_ash_log_top - same seed, same ring tones, same
    heartwood - on the same bark ramp. The only difference is the 1px border, which
    moves off the dark bark tones and up into the wood's own body, because that ring
    IS the bark and a stripped log has none.
    ...
    """
    c = Canvas(ASH_BARK_RAMP)
    log_rings(c, 8401, dark=1, bark_dark=4, bark_mid=3, heart=None, bark_speck=0.14)
    return c
...
def t_stripped_amber_bough_log_top() -> Canvas:
    ...
    So this is the SAME log_rings call as t_amber_bough_log_top - same seed, same ring tones, same
    heartwood - on the same bark ramp. The only difference is the 1px border, which
    moves off the dark bark tones and up into the wood's own body, because that ring
    IS the bark and a stripped log has none.
    ...
    """
    c = Canvas(BOUGH_BARK_RAMP)
    log_rings(c, 8443, dark=4, bark_dark=4, bark_mid=5, heart=7)
    return c
