...
def harmonic_axe() -> Sprite:
    ...
    for (x, y), lvl in haft_levels.items():
        lvl += 0.08 * (fbm(x * 2.1, y * 2.1, 7401, octaves=2) - 0.5)
        sp.put(x, y, NULL_IRON, NULL_IRON.tone(lvl))
    for (x, y), lvl in {**rear_lobe_levels, **front_blade_levels}.items():
        lvl += 0.08 * (fbm(x * 2.1, y * 2.1, 7407, octaves=2) - 0.5)
        sp.put(x, y, BISMUTH, BISMUTH.tone(lvl))
    sp.outline()
    # Explicitly set rear lobe outer pixels to Bismuth tone to ensure they read as Head material
    sp.stamp([(13, 6), (13, 7), (11, 8), (12, 8)], BISMUTH, 0.28)
    sp.stamp([(12, 6), (12, 7)], BISMUTH, 1.0)
    sp.stamp([(10, 6), (11, 6), (11, 7)], BISMUTH, 0.75)
    sp.stamp([(9, 6), (8, 7), (11, 4)], NULL_IRON, 1.0)
    sp.stamp([(8, 1), (7, 2), (6, 3), (5, 4), (5, 5), (5, 6), (6, 7)], BISMUTH, 1.0)
    sp.stamp([(7, 2), (5, 7)], GOLD, 1.0)
    # Ensure (10, 8) is clean empty gap
    if (10, 8) in sp.cell:
        del sp.cell[(10, 8)]
    return sp
...
def thrum_ribs() -> Sprite:
    """A diagonal rack of three curved thrum boar ribs with blunt bone ends and scalloped meat webbing."""
    hex_map = {
        "0": (22, 11, 30, 255),
        "1": (42, 20, 54, 255),
        "2": (58, 29, 71, 255),
...
def seared_thrum_ribs() -> Sprite:
    """A glazed, fire-roasted rack of three diagonal thrum ribs with caramelized golden-amber glaze and charred bone ends."""
    hex_map = {
        "0": (22, 11, 30, 255),
        "1": (86, 63, 35, 255),
        "2": (122, 92, 51, 255),
